...
public class PacienteService {
	...
	public Optional<PacienteModel> updateEntity(PacienteModel pacienteModel, UUID id) {
		Optional<PacienteModel> busca = pacienteRepository.findById(id);
		if (busca.isEmpty()) {
			return Optional.empty();
		}
		PacienteModel paciente = busca.get();
		paciente.setnomeCompleto(pacienteModel.getnomeCompleto());
		paciente.setDataNascimento(pacienteModel.getDataNascimento());
